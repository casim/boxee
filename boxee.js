if (!window.THE_BOXEE_BKMRKLT)
    window.THE_BOXEE_BKMRKLT = {
        server:'boxee.tv',
        logged_in:false,username:false,tab_el:false,border_el:false,close_el:false,toolbar_el:false,styles_el:false,ip_popup:false,cur_video_id:0,services:[],videos:[],log_id:false,library_interval:false,init:function(options) {
        this.debug('init');
        ...
        if (!document.getElementById('bkmrklt_boxee_toolbar'))
        ...
        if (this.videos.length == 0) {
            ...
            this.toolbar_el.innerHTML = '<span class="bkmrklt_boxee_close"  onclick="THE_BOXEE_BKMRKLT.toolbar_el.style.display = \'none\';">&times;</span><div style="width: 300px;">Sorry, Boxee couldn\'t find any videos on this page. <a class="info" href="http://' + this.server + '/watchlater" target="_blank">Click here for more info</a></div>';
            return false;
        }
        ...
        else if (this.logged_in && this.cur_video_id == undefined || this.cur_vid_id == 0) {
            this.jumpToVideo();
            this.selectViewportVideo();
        }
        else {
            this.debug('No objects found');
        }
        this.draw();
        jQuery(window).unbind('resize').unbind('scroll');
        jQuery(window).bind('scroll', {that:this}, this.onScroll);
        jQuery(window).bind('resize', {that:this}, this.onScroll);
        ...
        jQuery(window).bind('blur', {that:this}, this.windowOnBlur);
        jQuery(window).bind('focus', {that:this}, this.windowOnFocus);
        this.draw();
    },loggingIn:function() {
        jQuery('#bkmrklt_boxee_desc').html('<a href="javascript:void(0)" onclick="THE_BOXEE_BKMRKLT.loginCheck();">Click here once logged in</a>');
        this.draw();
    },loginCheck:function() {
        this.request('http://' + this.server + '/bookmarklet/login_check', {args:{callback:'loginCheckResponse'}});
                ...
                for (var k = 0; k < this.services[j].regex.length; k++) {
                    ...
                    if (r.test(str) && valid_domain) {
                        ...
                        if (pieces[1] == undefined)
                            pieces[1] = window.location.href;
                        this.videos.push({embed:obj,service_id:this.services[j].id,domain:this.services[j].domain,id:decodeURIComponent(pieces[1])});
                        break;
                    }
                }
            }
        ...
        if (this.videos.length == 0) {
            for (j = 0; j < this.services.length; j++) {
                ...
                if (scrape_r.test(window.location)) {
                    ...
                    this.videos.push({embed:false,service_id:this.services[j].id,domain:this.services[j].domain,id:decodeURIComponent(match[1])});
                    return;
                }
            }
        }
        ...
        var video = false;
        var videoOffset,new_videoOffset = 0;
        var new_temp,old_temp = 999999;
        var windowLocation = jQuery(window).scrollTop();
        var windowHeight = window.innerHeight;
        for (var i = 0; i < this.videos.length; i++) {
            if (this.videos[i] == undefined)
                continue;
            video = this.videos[i].embed;
            videoOffset = video.offset().top;
            new_temp = videoOffset - windowLocation;
            if (new_temp < 0) {
                new_temp = new_temp * -1;
            }
            if (new_temp < old_temp) {
                old_temp = new_temp;
                new_videoOffset = videoOffset;
            }
            else {
                videoOffset = 999999;
            }
        }
        if (new_videoOffset > 0 && new_videoOffset < windowHeight && new_videoOffset >= windowLocation)
            this.addToQueue(this.cur_video_id); else
            jQuery('body, html').animate({scrollTop:new_videoOffset - 20}, 300);
    },draw:function() {
        if (this.cur_video_id >= 0) {
            if (!this.logged_in) {
            }
            else if (this.videos[this.cur_video_id].adding_to_queue) {
            ...
            if (target !== false) {
                ...
                if (jQuery.browser.msie === true && this.tab_el.outerWidth() > 850) {
                    ...
                    this.tab_el[0].style.left = (pos.left + w - this.tab_el.outerWidth() + padding) + 'px';
                }
                ...
                var boxee_width = w;
                if (browser_width <= boxee_width) {
                    this.tab_el[0].style.left = '10px';
                    this.border_el[0].style.left = (pos.left) + 'px';
                    this.border_el[0].style.width = (w - 20) + 'px';
                }
            }
            else {
                this.tab_el[0].style.left = '10px';
        ...
        if (jQuery('#bkmrklt_boxee_tab').length == 0) {
            ...
            if (jQuery.browser.msie === true)
                this.styles_el.styleSheet.cssText = css; else
                jQuery(this.styles_el).html(css);
        ...
        this.border_el.bind('click', {that:this}, function(e) {
            e.data.that.close();
        });
        this.close_el.bind('click', {that:this}, function(e) {
            e.data.that.close();
        });
        this.tab_el.bind('click', {that:this,id:this.cur_video_id}, function(e) {
            if (e.data.that.logged_in)
                e.data.that.addToQueue(e.data.id);
        });
        this.draw();
        ...
        if (this.videos[i].added_to_queue)
            return false;
        if (jQuery.browser.msie) {
            var query = '?service_id=' + encodeURIComponent(this.videos[i].service_id) + '&id=' + encodeURIComponent(this.videos[i].id == undefined ? '' : this.videos[i].id) + '&url=' + encodeURIComponent(window.location.href) + '&log_id=' + this.log_id;
        ...
        this.request('http://' + this.server + '/bookmarklet/queue/add', {args:{callback:'addToQueueResponse',url:window.location.href,service_id:this.videos[i].service_id,id:this.videos[i].id == undefined ? '' : this.videos[i].id,log_id:this.log_id}});
        this.videos[i].adding_to_queue = true;
        this.draw();
    },popup_interval:false,addToQueuePopupResponse:function() {
        ...
        if (that.ie_popup.closed === true) {
            ...
            for (var i = 0; i < that.videos.length; i++) {
                if (that.videos[i].adding_to_queue == true) {
                    ...
                    that.videos[i].meta = {title:'Success!'};
                    that.draw();
                    break;
                }
            }
    ...
    },script:false,request:function(url, options) {
        if (this.script) {
    ...
    },debug_el:false,debug:function() {
        if (this.server == 'www.boxee.tv')
            return false;
...
THE_BOXEE_BKMRKLT.init({"logged_in":false,"username":false,"services":[
    {"id":"35","domain":"adultswim.com","scrape_url":"http:\\\/\\\/video\\.adultswim\\.com","regex":["tools\\\/swf\\\/sitevplayer.swf"]},
    {"id":"50","domain":"abc.go.com","scrape_url":"http:\\\/\\\/abc\\.go\\.com\\\/(.*?)","regex":["VP2\\.swf"]},
    {"id":"82","domain":"allthingsd.com","scrape_url":"","regex":["http:\\\/\\\/s\\.wsj\\.net\\\/media\\\/swf\\\/main\\.swf"]},
    {"id":"56","domain":"bbc.co.uk, news.bbc.co.uk","scrape_url":"","regex":["http:\\\/\\\/[a-zA-Z0-9]+\\.bbc\\.co\\.uk\\\/player\\\/emp\\\/[0-9_]+\\\/widgets\\\/[a-zA-Z0-9]+\\.swf\\?revision="]},
    {"id":"13","domain":"blip.tv","scrape_url":"http:\\\/\\\/(?:\\w+\\.)*blip\\.tv","regex":["flash\\\/stratos\\.swf","http:\\\/\\\/blip\\.tv\\\/play\\\/"]},
    {"id":"16","domain":"break.com","scrape_url":"http:\\\/\\\/(?:\\w+\\.)*break\\.com","regex":["sLink=(.*)&EmbedSEOLinkKeywords"]},
    {"id":"41","domain":"brightcove.com","scrape_url":false,"regex":["brightcove.com\\\/services\\\/viewer"]},
    {"id":"54","domain":"cbs.com","scrape_url":"http:\\\/\\\/www\\.cbs\\.com\\\/.*?|http:\\\/\\\/www\\.cbs\\.com\\\/(.*?)","regex":["rcpHolder"]},
    {"id":"74","domain":"cbsnews.com","scrape_url":"","regex":["http:\\\/\\\/www\\.cbsnews\\.com\\\/video\\\/watch\\\/\\?id=([0-9]+n)&releaseURL","([0-9]+)n&autoplay=true","rcpHolder"]},
    {"id":"21","domain":"collegehumor.com","scrape_url":"http:\\\/\\\/(?:\\w+\\.)*collegehumor\\.com\\\/video:([0-9]+)|http:\\\/\\\/(?:\\w+\\.)*collegehumor\\.com.*clip_id=([0-9]+)","regex":["videoid([0-9]+)","clip_id=([0-9]+)"]},
    {"id":"52","domain":"comedycentral.com, mtvnservices.com, media.mtvnservices.com","scrape_url":"http:\\\/\\\/media\\.mtvnservices\\.com\\\/(.*?)","regex":["http:\\\/\\\/media\\.mtvnservices\\.com\\\/(mgid:cms:.*?:.*?\\.com:[0-9]+)"]},
    {"id":"71","domain":"cwtv.com","scrape_url":"http:\\\/\\\/www\\.cwtv\\.com\\\/(.*?)","regex":["mn_player_QMP"]},
    {"id":"22","domain":"dailymotion.com","scrape_url":"http:\\\/\\\/(?:\\w+\\.)*dailymotion\\.com\\\/video\\\/(.*)","regex":["videoId%22%3A%22([a-zA-Z0-9]+)","dailymotion.com%2Fvideo%2F([a-zA-Z0-9]+)_","dailymotion\\.com\\\/swf\\\/([a-zA-Z0-9]+)","www.dailymotion.com\\\/video\\\/([a-zA-Z0-9]+)_"]},
    {"id":"51","domain":"espn.com, espn.go.com","scrape_url":"http:\\\/\\\/espn\\.go\\.com\\\/video\\\/clip\\?id=([0-9]+)&categoryid=([0-9]+)","regex":["ESPN_Player\\.swf\\?id=[0-9]+"]},
    {"id":"78","domain":"exercisetv.tv","scrape_url":"","regex":["playerwidget"]},
    {"id":"58","domain":"fora.tv","scrape_url":"","regex":["FORA_Player_[0-9]+\\.ver[0-9_]+\\.swf"]},
    {"id":"81","domain":"fubiz.net","scrape_url":"http:\\\/\\\/www\\.fubiz\\.net","regex":["mediaplayer\\.swf"]},
    {"id":"53","domain":"funimation.com, player.hulu.com","scrape_url":"http:\\\/\\\/www4\\.funimation\\.com\\\/video\\\/\\?page=video&v=(.*?)","regex":["videoTitle=.*?","FUNimationVideo","http:\\\/\\\/player\\.hulu\\.com\\\/express\\\/.*?"]},
    {"id":"26","domain":"funnyordie.com, funnyordie.co.uk","scrape_url":"http:\\\/\\\/(?:\\w+\\.)*funnyordie\\.com\\\/videos\\\/([a-zA-Z0-9]+)","regex":["key=([a-zA-Z0-9]+)"]},
    {"id":"77","domain":"gamespot.com","scrape_url":"","regex":["Swiff_[0-9]+"]},
    {"id":"27","domain":"gametrailers.com","scrape_url":false,"regex":["mid=([0-9]+)","%26id=([0-9]+)"]},
    {"id":"63","domain":"gawker.com","scrape_url":"","regex":["http:\\\/\\\/cache\\.gawkerassets\\.com\\\/assets\\\/util\\\/videoModule"]},
    {"id":"65","domain":"giantbomb.com","scrape_url":"http:\\\/\\\/www\\.giantbomb\\.com\\\/(.*?)","regex":["class=\"player-wrapper\"","videoUrl: \"(.*?)\"","http:\\\/\\\/media\\.giantbomb\\.com\\\/media\\\/video\\\/flash\\\/"]},
    {"id":"39","domain":"video.google.com","scrape_url":"","regex":["docid=([\\-0-9]+)"]},
    {"id":"24","domain":"howcast.com","scrape_url":"http:\\\/\\\/(?:\\w+\\.)*howcast\\.com\\\/videos\\\/(.*)","regex":["www.howcast.com\\\/videos\\\/([0-9]+)"]},
    {"id":"14","domain":"hulu.com","scrape_url":"http:\\\/\\\/(?:\\w+\\.)*hulu\\.com","regex":["\\\/site-player\\\/playerwrapper\\.swf"]},
    {"id":"38","domain":"ign.com","scrape_url":false,"regex":["\"href\":\"(.*?)\""]},
    {"id":"69","domain":"indieclick.com, indieclicktv.com","scrape_url":"http:\\\/\\\/www\\.indieclicktv\\.com\\\/(.*?)","regex":["(http:\\\/\\\/.*?\\.indieclicktv\\.com\\\/player\\\/swf\\\/[a-zA-Z0-9]+\\\/[a-zA-Z0-9]+\\\/[0-9]+\\\/[0-9]+\\\/defaultPlayer)-player\\.swf"]},
    ...
    {"id":"62","domain":"megavideo.com","scrape_url":"http:\\\/\\\/www\\.megavideo\\.com\\\/\\?v=(.*?)","regex":["http:\\\/\\\/wwwstatic\\.megavideo\\.com\\\/mv_player[0-9]\\.swf"]},
    ...
    {"id":"25","domain":"mtv.com","scrape_url":false,"regex":[":mtv.com:[0-9]+"]},
    {"id":"30","domain":"vids.myspace.com","scrape_url":false,"regex":["&amp;el=(.*)&amp;on","&el=(.*)&on"]},
    {"id":"67","domain":"nbc.com","scrape_url":"http:\\\/\\\/www\\.nbc\\.com\\\/(.*?)","regex":["rewindpl3_0"]},
    {"id":"46","domain":"video.pbs.org","scrape_url":"http:\\\/\\\/video\\.pbs\\.org\\\/video\\\/([0-9]+)","regex":["width=512&height=288&video=.*?\\\/([0-9]+)"]},
    {"id":"49","domain":"revision3.com","scrape_url":"http:\\\/\\\/revision3\\.com\\\/(.*?)","regex":["player-v([0-9]+)"]},
    {"id":"68","domain":"spike.com","scrape_url":"http:\\\/\\\/www\\.spike\\.com\\\/(.*?)","regex":["video_player_[0-9]+","PRODUCT_OBJECT"]},
    {"id":"84","domain":"techcrunch.tv","scrape_url":"","regex":["embedCode=(\\w*)"]},
    {"id":"33","domain":"ted.com","scrape_url":"","regex":["&amp;su=(http:\\\/\\\/www\\.ted\\.com.*?\\.html)&amp;","&su=(http:\\\/\\\/www\\.ted\\.com.*?\\.html)&","vu=http:\\\/\\\/video\\.ted\\.com\\\/.*?&su"]},
    {"id":"20","domain":"theonion.com","scrape_url":"","regex":["videoid=([0-9]+)"]},
    {"id":"75","domain":"travelchannel.com","scrape_url":"http:\\\/\\\/www\\.travelchannel\\.com\\\/(.*?)","regex":["videoPlayerWidget"]},
    {"id":"34","domain":"twit.tv, live.twit.tv","scrape_url":"","regex":["TWiT.*?"]},
    {"id":"40","domain":"vbs.tv","scrape_url":false,"regex":["permalink=(.*?)&","&pl=(.*?)\""]},
